...
elementid,
phase,
to_char(to_timestamp(lastmodifiedtime/1000), 'YYYY-MM-DD HH24:MI:SS.MS') as lastmodifiedtime,
case when caselist <> '' then caselist else NULL end as caselist
FROM calls 
WHERE ltrim(rtrim(upper(callstatus))) IN ('ACTIVE') 
AND calls.deleted=false 
ORDER BY caselist asc, ticketnum asc
-- LIMIT 500
		";
...
if ($calls_rs_row_count < 1) {
    ...
    echo "        <th nowrap align='left' colspan='".$calls_rs_field_count."'><h3>Active Calls (".$calls_rs_row_count.")</h3></th>\n";
    echo "    </tr><tr>\n";
