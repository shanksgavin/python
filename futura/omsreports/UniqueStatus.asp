...
<html>
<head>
<title>Reporting Unique Status Values and Counts</title>
<!--#include virtual="omsreports/dbConn.asp"-->
<link rel="stylesheet" type="text/css" href="OMSStyleSheet.css">
</head>
<%
dbConn.Open
sql = "select distinct(status) as status, count(status) as status_count from cases where deleted = '0' group by (status) order by status"
...
%>
<body>
<h1>OMS Unique Status Values and Counts</h1>
...
do until rs.EOF
  Response.Write("        <tr>")
  Response.Write("          <td style='width: 200px;'>" & rs.Fields.Item("status") & "</td>")
  Response.Write("          <td style='width: 200px;'>" & rs.Fields.Item("status_count") & "</td>")
  Response.Write("        </tr>")
  rs.MoveNext
loop
