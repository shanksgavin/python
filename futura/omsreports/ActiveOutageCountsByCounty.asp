...
<html>
<head>
<title>Reporting Active Outages By County</title>
...
function validateForm() {
    ...
    if (isNaN(startDate)) {
        ...
        alert("Start Date was not correctly formatted.\n Start Date readjusted to today: " + startDate1);
        //return startDate
    }
    if (isNaN(endDate)) {
        var ed1 = new Date();
        endDate1 = ed1.getFullYear() + addLeadingZero(ed1.getMonth() + 1) + addLeadingZero(ed1.getDate());
        fObj.elements["endDate"].value = endDate1;
        alert("End Date was not correctly formatted.\n End Date readjusted to today: " + endDate1);
    }
    if (startDate > endDate) {
        fObj.elements["startDate"].value = endDate;
        alert("Start Date cannot be after End Date.\n Start Date was changed to End Date.");
...
dbConn.Open
sql = "SELECT DISTINCT(county) as county, COUNT(county) as outages from (select cc.customer, m.elementid, m.county, cc.deleted,cc.casenum from casescustomers cc, meterbase m, cases c where cc.customer = m.elementid AND cc.casenum = c.casenum AND c.casestatus in ('CauseFound', 'CauseUnknown', 'Predicted') AND cc.deleted = 'f' AND (c.datestrt>='" & startDate & "' and c.dateend<='" & endDate & "')) AS customers GROUP BY customers.county ORDER BY customers.county ASC"
set rs = Server.CreateObject("ADODB.recordset")
rs.Open sql, dbConn
%>
<body>
<h1>OMS Active Outages By County</h1>
