            ...
            if lyr.isGroupLayer == False:
                ...
                if lyr.supports("LABELCLASSES"):
                    ...
                    for lblClass in lyr.labelClasses:
                        ...
                        print("    {0} - Label Class: {1}".format(sub_counter,lblClass.className))
                        sub_counter += 1
                        ...
                        if sql_found_open_sq_brackets or sql_found_closed_sq_brackets or sql_found_wildcard:
                            ...
                            print("        Updated SQL Query: {0}".format(lblClass.SQLQuery))
#                         else:
#                             if logging:
...
#                             print("        No SQL Updates Needed...")
                        ...
                        if len(lblClass.SQLQuery) > 0:
                            if lyr.supports("DATASOURCE") and logging:
                                ...
                                print("    Layer supports dataSource property: {0}".format(lyr.supports("DATASOURCE")))
                                print("    Logging is set to {0}".format(logging))
...
#                         exp_found_open_sq_brackets = False
#                         exp_found_closed_sq_brackets = False
#                         exp_found_wildcard = False
#                         if lblClass.expression.find("[") != -1:
#                             exp_found_open_sq_brackets = True
#                             lblClass.expression = lblClass.expression.replace("[", "\"")
#                         if lblClass.expression.find("]") != -1:
#                             exp_found_closed_sq_brackets = True
#                             lblClass.expression = lblClass.expression.replace("]", "\"")
#                         if lblClass.expression.find("*") != -1:
#                             exp_found_wildcard = True
#                             lblClass.expression = lblClass.expression.replace("*", "%")
#                         
#                         if exp_found_open_sq_brackets or exp_found_closed_sq_brackets or exp_found_wildcard:
#                             if logging:
#                                 log.write("        Updated Expression: {0}\n".format(lblClass.expression))
#                             print("        Updated Expression: {0}".format(lblClass.expression))
#                         else:
#                             if logging:
#                                 log.write("        No Expression Updates Needed...\n")
#                             print("        No Expression Updates Needed...")
...
def verifySQLFields(layer, sql):
    ...
    for comp in sqlquery[1:]:
        if sqlquery.index(comp)%2 != 0:
            ...
            if Matched == False:
                results.append([comp, "SQL_FIELDS_NOT_MATCHED"])
    if len(results) == 0:
        results = None
    ...
    while len(exp) > 0:
        ...
        for field in fieldList:
            if expression_field.lower() == field.name.lower():
                Matched = True
        if Matched == False:
            results.append([expression_field, "EXP_FIELDS_NOT_MATCHED"])
...
if __name__ == '__main__':
    #printMXDLayers("C:\\map_files_cwf_101\\Update_of_Coweta_fgdb.mxd")
    updateLabelClasses("C:\\map_files_cwf_101\\Coweta_sde.mxd", True)
